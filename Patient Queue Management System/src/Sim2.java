...
public class Sim2 {
	...
	private static void check_emergency() {
		...
		if (random.nextDouble() < Parameters.p_emergency) {
			...
			// -----
			int rand_Q = random.nextInt(Q.length);
			int rand_patient = random.nextInt(Q[rand_Q].size);
			Patient p = Q[rand_Q].dequeue(rand_patient);
			Q[rand_Q].push(p);
		}
	}
	...
	private static void check_promotion() {
		...
		for (int i = 0; i < Q[0].size; i++) { // check long-waiting patients from queue 1
			if (Q[0].get(i).wait_time(time) >= Parameters.l12_escalation_delay) {
				Q[1].push(Q[0].dequeue(i));
			}
		}

		for (int i = 0; i < Q[1].size; i++) { //check long waiting patients from queue 2
	...
	private static void check_walkout() {
		...
		if (random.nextDouble() < Parameters.p_walkout) {
			...
			// -----
			Q[0].dequeue(random.nextInt(Q[0].size)); // dequeue rand patient in level 1
		}
	}
